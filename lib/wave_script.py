...
if method == 'leapfrog':
    previous_solution = numpy.copy(current_solution)

t = 0
while t < t_max + 2 * t_step:#why this?
    if method == 'lax_wendroff':
        current_solution = wave.lax_wendroff(current_solution, speed, x_step, t_step)
    elif method == 'lax_friedrichs':
        current_solution = wave.lax_friedrichs(current_solution, speed, x_step, t_step)
    elif method == 'leapfrog':
        previous_solution, current_solution = current_solution, wave.leapfrog(current_solution, previous_solution, speed, x_step, t_step)
    else:
        print('MACHEOOOHHHH!!!')
        quit
    if results == 'norm':
